/*
  ...
  Sampling when using the Wire library, <wire.h>, was found to be unreliable over time
  Converted to use I2C library since it handles the repeated START I2C commands that the 
  Freescale, Inc., devices appear to use
  ...
  Usage:
    ...
    -Monitors status flags used to poll device for data ready
 
  During testing, GPS with 9 satellites reported 5393ft, sensor reported 5360ft 
    (delta of 33ft). Very close! (Seidle)
  During testing in Bozeman, GPS with 9 satellites reported 1520m; sensor reported 1433m; 
...
// ALTITUDE CORRECTION (IF DESIRED)  
// try with MY_ALTITUDE set to 0 and see if altimeter is
// accurate; if not, set MY_ALTITUDE to GPS-provided altitude in meters
  if (MY_ALTITUDE != 0) thing.setAltitude(MY_ALTITUDE);
